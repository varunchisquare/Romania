...
/*select dat,
sum(case Type when 'deposit' then amt end ) deposit,
sum(case Type when 'withdraw' then amt end ) withdraw from (
select date(Requestdate) dat,  CURRENCYCODE,Type ,sum(amount) amt
from romania.daily_player_transactions
where CURRENCYCODE = 'RON'
and status = 'approved'
group by date(Requestdate),  CURRENCYCODE,Type) ab
group by dat;


select distinct type from romania.daily_player_transactions*/
